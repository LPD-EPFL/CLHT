...
static inline void 
lock_release_n(bucket_t* b, int n)
{
  int i;
  for (i = 0; i < n; i++)
    {
...
ht_put(hyht_wrapper_t* h, hyht_addr_t key, hyht_val_t val) 
{
 ...
 again:
  ...
  uint32_t* bucket_hops = (uint32_t*) &bucket->hops;
  while (hops-- >= 0)
    {
      locks[l] = &bucket->lock;
      if (!LOCK_ACQ(locks[l], hashtable))
	{
	  printf("|PUT| failed due to resize!\n");
	  for (j = 0; j <= l; j++)
	    {
	      LOCK_RLS(locks[j]);
	    }
  ...
  while (hops-- >= 0)
    {
      ...
      if (!LOCK_ACQ(locks[l], hashtable))
	{
	  printf("|REM| failed due to resize!\n");
	  for (j = 0; j <= l; j++)
	    {
	      LOCK_RLS(locks[j]);
	    }
